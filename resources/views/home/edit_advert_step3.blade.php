...
@section('content')
    ...
    <div class="body row">
        ...
        @endif
        <form class="col-xs-12 body" method="POST" action="{{secure_url('/home/adverts/add/step3')}}">
            {{csrf_field()}}
            ...
            <div class="col-xs-12 col-sm-6 col-md-4 margin-top-always">
                <div class="fluid ui buttons">
                    <button class="text-left ui teal basic button">
                        <i class="video icon"></i>Видео ({{$advert->videos->count()}})
                    </button>
                ...
                <div class="col-xs-12 videoBlock"></div>
                <!-- Модальное окно -->
                        ...
                        <div class="modal-content">
                            ...
                            <div class="modal-footer">
                                <div class="ui buttons">
                                    ...
                                    <button type="button" class="ui positive button">Сохранить изменения</button>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div id="dropMusics" class="col-xs-12 musicBlock">
                    ...
                    <div class="readyMusics">
                        @foreach($advert->musics as $music)
                            <div id="readyMusic-{{$advert->id}}-{{$music->id}}" class="readyMusic">
                                <audio src="{{secure_asset($music->path)}}" controls class="col-xs-12"></audio>
                                <span class="col-xs-12">{{$music->name}}</span>
                                <a id="drop-{{$advert->id}}-{{$music->id}}" class="dropInMusic2" title="Delete"><i class="remove icon" title="Удалить аудио"></i></a>
                            </div>
                        @endforeach
                    </div>
                    <input style="display: none;" type="file" name="uploadInputMusics" id="uploadInputMusics" class="inputfile" multiple/>
                    <div class="loadingMusics">
