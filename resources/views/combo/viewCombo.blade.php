...
@section('content')
    <div class="body">
        ...
        <div class="col-xs-12 col-sm-3">
            <div class="col-xs-12 padding-0-always"
                 style="background: rgba(111, 151, 213, .6); border-radius: 54px 6px 0 54px;">
            ...
            <ul class="col-xs-12 margin-top-always nav nav-tabs text-center" role="tablist">
                ...
                @foreach($combo_cit->combo_categors as $combo_categor)
                    <li role="presentation" class="col-xs-12 {{$stage}}">
                        <a href="#{{$combo_categor->id}}" aria-controls="{{$combo_categor->id}}" role="tab"
                           data-toggle="tab">{{$combo_categor->advert_categor->name}}</a>
                    </li>
                    <?php $stage = '';?>
                @endforeach
                <a href="#" class="btn btn-default margin-top-always margin-bottom-always setCombo">Оформить
                    заявку</a>
            </ul>
        </div>
        <div class="col-xs-12 col-sm-9 padding-0-always tab-content">
            ...
            @foreach($combo_cit->combo_categors as $combo_categor2)
                <div role="tabpanel" class="tab-pane {{$stage}}" id="{{$combo_categor2->id}}">
                    <h3 class="margin-top-always margin-bottom-always text-center">Имя
                        категории: {{$combo_categor2->advert_categor->name}}</h3>
                    <div class="ui three stackable special cards">
                        <?php $checked = 'checked'; ?>
                        @foreach($combo_categor2->adverts as $advert)
                            <div class="card baseAdvDiv-{{$combo_categor2->id}}" id="baseAd-{{$advert->id}}">
                                        ...
                                        <div class="content">
                                            <div class="center">
                                                ...
                                                @foreach($advert->photos as $photo)
                                                    <div class="col-xs-3 photo-advert"
                                                         style="background-image: url({{secure_asset($photo->path)}});"></div>
                                                @endforeach
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="extra content">
                                            <span style="float: left;"><i
                                                        class="unhide icon"></i> {{$advert->views}} </span>
                                    <span style="float: right;"><i
                                                class="star icon"></i> {{$advert->rating}} </span>
                                </div>
                                <div class="extra content">
                                    <div class="ui two buttons">
                                                <span id="dekBut-{{$combo_categor2->id}}-{{$advert->id}}"
                                                      class="ui basic green button dekBut">
                                                    <input id="{{$advert->id}}"
                                                           class="radioAdv radio-{{$combo_categor2->id}}"
                                                           {{$checked}} name="{{$combo_categor2->id}}"
                                                           type="radio"
                                                           value="{{$advert->id}}"
                                                           style="width: 100%;height: 100%;position: absolute;visibility: hidden;margin: 0;left: 0;top: 0;">Выбрать
                                                </span>
                                        <a href="{{secure_url('advert/'.$advert->id)}}"
                                           class="ui basic teal button">Подробнее</a>
                                    </div>
                                </div>
                            </div>
                            <?php $checked = ''; ?>
